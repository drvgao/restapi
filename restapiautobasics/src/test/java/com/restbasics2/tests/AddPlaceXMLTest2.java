...
/**
 * create a place. Extract the placeId from XML response. use custom methods get
 * xmlpath.
 * 
 * @author RDURGAM
 *
 */
public class AddPlaceXMLTest2 {
	Properties prop;
	...
	public void test() {
		try {
			System.out.println(
					"string: " + Utils.getXMLString("./src/test/java/com/restbasics2/tests/AddPlacePostData.xml"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void addPlaceXML() throws IOException {
		...
		XmlPath xmlPath = Utils.rawToXML(response);
